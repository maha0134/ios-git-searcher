...
//MARK: propery wrappers
    ...
    var body: some View {
        VStack{
            Text("Tune up").font(.title)
            Stepper(value:$resultsPerPage, in: 0...50, step:1){
                Text("Results per page: \(resultsPerPage)")
            }
            Stepper("Minimum repos: \(minimumRepos)", value: $minimumRepos, in: 0...100,step: 5)
            Stepper("Minimum followers: \(minimumFollowers)", value: $minimumFollowers, in: 0...100,step: 5)
            Spacer()
        }.padding()
    }
}
